 ...
 * Use AllPunctaFuncvC405 & SinglePunctaFunctionvC404 
...
// v301. adapt to work in background on a folder of files.


	dir1 = getDirectory("Where are your images saved?.."); //v301
	list = getFileList(dir1); //make a list of the filenames

	savelocation1 = getDirectory("Choose a Directory to save the ROIs");	

	for (i=0; i<list.length; i++) { //v301
	...
	File.makeDirectory(savelocation1 + "\\" + Shortname + "\\"); // make a new folder for the results. v301
	savelocation = savelocation1 + "\\" + Shortname + "\\"; // make the save location the new folder. v301

// work in the background
	setBatchMode("hide"); //v301
...
// v308 subtract BG on orig image now to make all measures absolute?? Or better still use the BG image made earlier
	...
	count=roiManager("count");																//v404
	if (count != 0) {roiManager("deselect");												//v404
	roiManager("deselect");
